...
class TestLink(SharedSetupTransactionCase):
    ...
    def initTestData(cls):
        ...
        cls.projet_us = cls.TICKET.create({
            'name': 'Test us',
            'participant_ids':
                [(6, 0, [cls.member_id, cls.customer_id, cls.partner_id])],
    ...
    def test_create_link_by_member(self):
        ...
        # Search by link id
        link_by_id = self.LINK.with_context(active_id=self.ticket_us_1).sudo(
            self.member_id).search([('id', '=', link_1.id)])
    ...
    def test_read_authorized_link_partner(self):
        ...
        # Search by link id
        link_by_id = self.LINK.with_context(active_id=self.ticket_us_1).sudo(
            self.partner_id).search([('id', '=', link_1.id)])
    ...
    def test_read_authorized_link_custommer(self):
        ...
        # Search by link id
        link_by_id = self.LINK.with_context(active_id=self.ticket_us_1).sudo(
            self.customer_id).search([('id', '=', link_1.id)])
